...
def fix_html_file(html_file, expected_labels, question_num):
    ...
    # Find all SVGs that don't have a parent with a label
    ...
    # Find all canvases that don't have a parent with a label
        ...
        for i, (elem_type, element) in enumerate(visual_elements):
            if i < len(sorted_labels):
                ...
                if elem_type == 'div':
                    ...
                    new_div = soup.new_tag('div', attrs={'class': 'visual-element item', 'label': label})

                    # Insert the new div where the element was
                    element.insert_before(new_div)

                    # Move the element into the new div
                    element.extract()
