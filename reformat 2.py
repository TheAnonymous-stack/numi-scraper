...
files = [
    "FORMAT_UPDATE2 - gr8MasterQuestions(C.8-CC.9).json",
    "FORMAT_UPDATE2 - gr8MasterQuestions(T.3-L.10).json",
    "FORMAT_UPDATE2- gr8MasterQuestions(H.1-D.24).json"
]

new_files = [
    "FORMAT_UPDATE3 - gr8MasterQuestions(C.8-CC.9).json",
    "FORMAT_UPDATE3 - gr8MasterQuestions(T.3-L.10).json",
    "FORMAT_UPDATE3- gr8MasterQuestions(H.1-D.24).json"
]
for i, file in enumerate(files):
    ...
    for question in data:
        ...
        if "image_tag" in question:
            ...
            new_image_tag = f"Gr8_{new_week_number}_{new_exercise_number}_{variation_number}"
            os.makedirs(f"New_Grade8_Master_Images/{new_week_number}", exist_ok=True)
            shutil.copy2(f"Grade8_Master_Images/{old_week_number}/{old_image_tag}.png",
                         f"New_Grade8_Master_Images/{new_week_number}/{new_image_tag}.png")
        ...
        if "image_choice_tags" in question:
            ...
            for old_image_tag in old_image_choice_tags:
                ...
                new_image_tag = f"Gr8_{new_week_number}_{new_exercise_number}_{variation_number}_{choice}"
                os.makedirs(f"New_Grade8_Master_Images/{new_week_number}", exist_ok=True)
                shutil.copy2(f"Grade8_Master_Images/{old_week_number}/{old_image_tag}.png",
                             f"New_Grade8_Master_Images/{new_week_number}/{new_image_tag}.png")
        ...
        if "solution_image_tag" in question:
            ...
            for obj in old_solution_image_tag:
                ...
                new_image_tag = f"Gr8_{new_week_number}_{new_exercise_number}_{variation_number}_step_{step_number}"
                os.makedirs(f"New_Grade8_Master_Images/{new_week_number}", exist_ok=True)
                shutil.copy2(f"Grade8_Master_Images/{old_week_number}/{old_image_tag}.png",
                             f"New_Grade8_Master_Images/{new_week_number}/{new_image_tag}.png")
