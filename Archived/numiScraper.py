...
def process_visual_components(page, json, code):
    ...
    try:
        ...
        if table:
            table.screenshot(path=f"Grade6_Master_Images/Grade6_{code.split('.')[0]}/Grade6_{base_code}/Grade6_{code}.png")
        ...
        if canvas:
            ...
            canvas.screenshot(path=f"Grade6_Master_Images/Grade6_{code.split('.')[0]}/Grade6_{base_code}/Grade6_{code}.png")
        ...
        if diagramWrapper:
            diagramWrapper.screenshot(path=f"Grade6_Master_Images/Grade6_{code.split('.')[0]}/Grade6_{base_code}/Grade6_{code}.png")
            
            json["image_tag"] = f"Grade6_{code}"

        fractionBar = section.query_selector("div.fractionBarBlockTable")
        if fractionBar:
            fractionBar.screenshot(path=f"Grade6_Master_Images/Grade6_{code.split('.')[0]}/Grade6_{base_code}/Grade6_{code}.png")
            
            json["image_tag"] = f"Grade6_{code}"

        selectableGridContainer = section.query_selector("div.selectableGridContainer")
        if selectableGridContainer:
            selectableGridContainer.screenshot(path=f"Grade6_Master_Images/Grade6_{code.split('.')[0]}/Grade6_{base_code}/Grade6_{code}.png")
        ...
        if stripContainer:
            ...
            stripContainer.screenshot(path=f"Grade6_Master_Images/Grade6_{code.split('.')[0]}/Grade6_{base_code}/Grade6_{code}.png")
        ...
        if multiplicationModelContainer:
            ...
            multiplicationModelContainer.screenshot(path=f"Grade6_Master_Images/Grade6_{code.split('.')[0]}/Grade6_{base_code}/Grade6_{code}.png")
            ...
            with sync_playwright() as p:
                ...
                if check_fill_in_the_blank(page):
                    if not check_duplicate(tracker["Fill in the blank"], text):
                        ...
                        section.screenshot(path=f"Grade6_Questions/Grade6_{base_code.split('.')[0]}/Grade6_{base_code}/Grade6_{code}.png")
                        process_visual_components(page, json, code)
                        json["question_type"] = "Fill in the blank"
                        json["question_number"] = code
                        json["tag"] = f"Grade6_{base_code}"
...
urls = ["https://ca.ixl.com/math/grade-6/compare-savings-options"]

scraped_questions = []
for url in urls:
    scrape_question(url, {}, scraped_questions)
write_to_json(scraped_questions, "gr6Draft.json")
