...
def rebuild_gr7_14_e2():
    ...
    for question in data['quizzes']:
        ...
        # Extract ONLY the equation between the first pair of $ symbols
        # Pattern: Find everything between the first $ and the next $, stopping at the period
        equation_match = re.search(r'\$(.*?)\$', q_text)
        if not equation_match:
            continue

        equation = equation_match.group(1)  # This is the LaTeX equation
