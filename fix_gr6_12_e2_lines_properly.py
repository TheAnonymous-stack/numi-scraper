...
def fix_equation_lines(text):
    """Fix the question text to have each equation on its own line with blanks"""
    ...
    if match:
        ...
        # Create properly formatted question with each line complete
        # Each blank (_____) is on its own and equations are complete per line
...
def process_file():
    ...
    for quiz in data['quizzes']:
        if 'question_text' in quiz:
            ...
            # Fix the formatting
            fixed = fix_equation_lines(original)
