...
def process_files():
    """Process all Grade 7 files from week 15 to 23."""
    changes_log = []
    files_processed = []
    html_files_created = []
    ...
    for week in range(15, 24):
        for exercise in range(1, 6):
            filename = f'Gr7_{week}_E{exercise}_variations.json'
            ...
            try:
                ...
                if file_changed:
                    ...
                    print(f"  Generated {len([f for f in html_files_created if f'_{week}_{exercise}_' in f])} HTML files")
                else:
...
def generate_report(changes_log: List[Dict], files_processed: List[str],
    ...
    report = "# Grade 7 Math Questions Update Report\n\n"
    report += f"## Summary\n"
    report += f"- Files processed: {len(files_processed)}\n"
    report += f"- Files with updates: {len([c for c in changes_log if 'questions_updated' in c])}\n"
    ...
    for change in changes_log:
        ...
        elif 'questions_updated' in change:
            report += f"\n### {change['file']}\n"
            report += f"- Questions updated: {change['total_updates']}\n"
            if change['total_updates'] > 0:
    ...
    report += "The following types of answer format instructions were added:\n"
...
if __name__ == "__main__":
    print("Starting Grade 7 Math Questions Update (Weeks 15-23)")
    print("=" * 50)
    ...
    report_filename = "gr7_15_23_update_report.md"
