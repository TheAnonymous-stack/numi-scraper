...
def extract_answer_fill_in_the_blank(page, json, code):
    ...
    try:
        ...
        section.screenshot(path=f"Grade5_Solutions/Grade5_{base_code.split(".")[0]}/Grade5_{base_code}/Grade5_{code}.png")
    ...
    try:
        ...
        if len(image_options) > 0:
            ...
            for i, option in enumerate(image_options):
                option.screenshot(path=f"Grade5_Master_Images/Grade5_{base_code.split('.')[0]}/Grade5_{base_code}/Grade5_{code}_{chr(i+65)}.png")
...
def extract_answer_drag_and_drop(page, json, code):
    ...
    try: 
        ...
        folder_name = f"Grade5_Master_Images/Grade5_{code.split('.')[0]}/Grade5_{code}"
    ...
    try:
        ...
        if answer:
            ...
            answer.screenshot(path=f"Grade5_Master_Images/Grade5_{code.split('.')[0]}/Grade5_{code}_answer.png")
...
def extract_answer_pattern_drag_and_drop(page, json, code):
    ...
    for i, stack in enumerate(stacks):
        ...
        pathName = f"Grade5_Master_Images/Grade5_{code.split('.')[0]}/Grade5_{code}_{chr(i + 65)}.png"
    ...
    try:
        ...
        section = page.query_selector("section.tab-box.web.optional-tab-box.solve-box section.ixl-practice-crate")
        section.screenshot(path=f"Grade5_Solutions/Grade5_{code}.png")
...
def extract_answer_explanation_with_images(page, json, code):
    try:
        ...
        for i, option in enumerate(res):
            res.screenshot(path=f"Grade5_Master_Images/Grade5_{code.split('.')[0]}/Grade5_{code}_solution_{i}.png")
...
def extract_answer_explanation(page, json, code):
    ...
    if table:
        table.screenshot(path=f"Grade5_Master_Images/Grade5_{base_code.split('.')[0]}/Grade5_{base_code}/Grade5_{code}_solution_step_{i}.png")
        json["solution_image_tag"].append(f"Grade5_{code}_solution_step_{i}")
        i += 1

    canvases = section.query_selector_all("canvas")
    if len(canvases) > 0:
        for canvas in enumerate(canvases):
            canvas.screenshot(path=f"Grade5_Master_Images/Grade5_{base_code.split('.')[0]}/Grade5_{base_code}/Grade5_{code}_solution_step_{i}.png")
            json["solution_image_tag"].append(f"Grade5_{code}_solution_step_{i}")
            i += 1
    ...
    #         svg.screenshot(path=f"Grade5_Master_Images/Grade5_{code.split('.')[0]}/Grade5_{code}_{i}")
    #         i += 1
    diagramWrappers = section.query_selector_all("div.diagramWrapper")
    if len(diagramWrappers) > 0:
        for wrapper in diagramWrappers:
            wrapper.screenshot(path=f"Grade5_Master_Images/Grade5_{base_code.split('.')[0]}/Grade5_{base_code}/Grade5_{code}_solution_step_{i}.png")
            json["solution_image_tag"].append(f"Grade5_{code}_solution_step_{i}")
            i += 1
    ...
    if len(fractionBars) > 0:
        for bar in fractionBars:
            bar.screenshot(path=f"Grade5_Master_Images/Grade5_{base_code.split('.')[0]}/Grade5_{base_code}/Grade5_{code}_solution_step_{i}.png")
            json["solution_image_tag"].append(f"Grade5_{code}_solution_step_{i}")
            i += 1
    ...
    if len(selectableGridContainers) > 0:
        for container in selectableGridContainers:
            container.screenshot(path=f"Grade5_Master_Images/Grade5_{base_code.split('.')[0]}/Grade5_{base_code}/Grade5_{code}_solution_step_{i}.png")
            json["solution_image_tag"].append(f"Grade5_{code}_solution_step_{i}")
            i += 1
