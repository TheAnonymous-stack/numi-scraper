...
def extract_answer_fill_in_the_blank(page, json, code):
    ...
    try:
        ...
        section.screenshot(path=f"Grade6_Solutions/Grade6_{code}.png")
    ...
    try:
        ...
        if len(image_options) > 0:
            ...
            for i, option in enumerate(image_options):
                option.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{code}_{chr(i+65)}.png")
                json["image_choice_tags"].append(f'Grade6_{code}_{chr(i+65)}')
        else:
            json["choices"] = []
...
def extract_answer_drag_and_drop(page, json, code):
    ...
    try: 
        ...
        folder_name = f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{code}"
    ...
    try:
        ...
        if answer:
            ...
            answer.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{code}_answer.png")
            json["correct_answers_image_tag"] = f"Grade6_{code}_answer"
...
def extract_answer_pattern_drag_and_drop(page, json, code):
    ...
    for i, stack in enumerate(stacks):
        ...
        pathName = f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{code}_{chr(i + 65)}.png"
...
def process_visual_components(page, json, code):
    ...
    try:
        ...
        if table:
            table.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{base_code}/Grade6_{code}.png")
            
            json["image_tag"] = f"Grade6_{code}"

        canvas = section.query_selector("canvas")
        if canvas:
            
            canvas.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{base_code}/Grade6_{code}.png")
        ...
        if diagramWrapper:
            diagramWrapper.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{base_code}/Grade6_{code}.png")
            ...
            if len(svgs) > 0:
                for svg in enumerate(svgs):
                    svg.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{code}_{i}")
                    i += 1
        fractionBar = section.query_selector("div.fractionBarBlockTable")
        if fractionBar:
            fractionBar.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{base_code}/Grade6_{code}.png")
            
            json["image_tag"] = f"Grade6_{code}"

        selectableGridContainer = section.query_selector("div.selectableGridContainer")
        if selectableGridContainer:
            selectableGridContainer.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{base_code}/Grade6_{code}.png")
        ...
        if stripContainer:
            ...
            stripContainer.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{base_code}/Grade6_{code}.png")
        ...
        if multiplicationModelContainer:
            ...
            multiplicationModelContainer.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{base_code}/Grade6_{code}.png")
        ...
        if imageWrapper:
            ...
            imageWrapper.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{base_code}/Grade6_{code}.png")
...
def extract_answer_explanation_with_images(page, json, code):
    try:
        ...
        for i, option in enumerate(res):
            res.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{code}_solution_{i}.png")
            json["solution_image_tag"] = f"Grade6_{code}_solution_{i}"
...
def extract_answer_explanation(page, json, code):
    ...
    table = section.query_selector("table")
    if table:
        table.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{code}_solution_step_{i}.png")
        json["solution_image_tag"].append(f"Grade6_{code}_solution_step_{i}")
        i += 1

    canvases = section.query_selector_all("canvas")
    if len(canvases) > 0:
        for canvas in enumerate(canvases):
            canvas.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{code}_solution_step_{i}.png")
            json["solution_image_tag"].append(f"Grade6_{code}_solution_step_{i}")
            i += 1
    ...
    if len(diagramWrappers) > 0:
        for wrapper in diagramWrappers:
            wrapper.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{code}_solution_step_{i}.png")
            json["solution_image_tag"].append(f"Grade6_{code}_solution_step_{i}")
            i += 1
    ...
    if len(fractionBars) > 0:
        for bar in fractionBars:
            bar.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{code}_solution_step_{i}.png")
            json["solution_image_tag"].append(f"Grade6_{code}_solution_step_{i}")
            i += 1
    ...
    if len(selectableGridContainers) > 0:
        for container in selectableGridContainers:
            container.screenshot(path=f"Grade6_Images/Grade6_{code.split('.')[0]}/Grade6_{code}_solution_step_{i}.png")
            json["solution_image_tag"].append(f"Grade6_{code}_solution_step_{i}")
            i += 1
