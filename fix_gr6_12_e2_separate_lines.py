...
def fix_question_with_separate_answer_lines(text):
    """Fix the question text to have each answer box completely on its own line"""
    ...
    if match:
        ...
        # Create properly formatted question with answer boxes on separate lines
...
def fix_answers_for_proper_order(quiz, num1, num2, num3):
    ...
    # First answer: the number we're moving to the middle position
    if n1 <= n3:
        first_answer = num1
...
def process_file():
    ...
    for quiz in data['quizzes']:
        if 'question_text' in quiz:
            ...
            if match:
                ...
                fixed = fix_question_with_separate_answer_lines(original)
    ...
    print("Each answer box is now completely on its own line with hints")
