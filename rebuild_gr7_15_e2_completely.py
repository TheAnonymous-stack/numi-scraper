...
def rebuild_gr7_15_e2():
    """Completely rebuild Gr7_15_E2 to be all subtraction with proper formatting"""
    ...
    for idx, question in enumerate(data['quizzes'], 1):
        ...
        # Update question type based on result
        if result.denominator == 1 or result < 0:
            # Whole number or negative result = Fill in the blank
            question['question_type'] = 'Fill in the blank'
            if result.denominator == 1:
                question['correct_answers'] = [str(result.numerator)]
            else:
                question['correct_answers'] = [f"{result.numerator}/{result.denominator}"]
        else:
            # Positive fraction = Multiple fill in the blank
        ...
        question['solution'] = [
            ...
            [
                "4/4",
                f"Therefore, $\\frac{{{num1}}}{{{den1}}} - \\frac{{{num2}}}{{{den2}}} = \\frac{{{diff_num}}}{{{lcd}}} = {result.numerator}/{result.denominator}$." if abs(diff_num) != abs(result.numerator) or lcd != result.denominator else f"Therefore, $\\frac{{{num1}}}{{{den1}}} - \\frac{{{num2}}}{{{den2}}} = {result.numerator}/{result.denominator}$." if result.denominator != 1 else f"Therefore, $\\frac{{{num1}}}{{{den1}}} - \\frac{{{num2}}}{{{den2}}} = {result.numerator}$."
            ]
        ]

        print(f"  Question {idx}: {num1}/{den1} - {num2}/{den2} = {result.numerator}/{result.denominator if result.denominator != 1 else ''}{result.numerator if result.denominator == 1 else ''}")
    ...
    print(f"\nSuccessfully rebuilt all {len(data['quizzes'])} questions to subtraction!")
