...
def create_standalone_svg_html(svg_content, title="Image"):
    """Create a complete standalone HTML file with just the SVG"""
...
def create_main_html_only(quiz):
    """Create main HTML with just the question text - no images"""
...
<body>
    <div class="container">
        ...
        <div class="info">
            View the associated image files:
            <br>• {tag}_{question_number}_question.html
            <br>• {tag}_{question_number}_solution_*.html
        </div>
    </div>
</body>
</html>"""
...
def process_quiz(quiz, output_dir):
    ...
    # 1. Create main HTML (just question text)
    main_html = create_main_html_only(quiz)
    main_file = os.path.join(output_dir, f"{tag}_{question_number}.html")
    with open(main_file, 'w', encoding='utf-8') as f:
        f.write(main_html)
    files_created.append(main_file)
    
    # 2. Create question image HTML (standalone)
    question_svg = create_coordinate_plane_svg(backend_desc, highlight_point)
    question_html = create_standalone_svg_html(question_svg, f"{tag} {question_number} Question")
    question_file = os.path.join(output_dir, f"{tag}_{question_number}_question.html")
    with open(question_file, 'w', encoding='utf-8') as f:
        f.write(question_html)
    files_created.append(question_file)
    
    # 3. Create solution step HTML files (each step separate)
    for i, step_info in enumerate(solution_steps):
        ...
        # Create HTML for this solution step text only
...
<body>
    ...
    # 4. Create solution image HTML files (if they exist)
    for i, img_info in enumerate(solution_image_tags):
        ...
        solution_html = create_standalone_svg_html(solution_svg, f"{tag} {question_number} Solution {step_num}")
...
def update_json_with_file_references(json_file):
    """Update JSON to include actual file references"""
    ...
    for quiz in data['quizzes']:
        ...
        # Add file references
        quiz['html_files'] = {
            'main': f"{tag}_{question_number}.html",
            'question_image': f"{tag}_{question_number}_question.html",
            'solution_steps': [],
            'solution_images': []
        }
        
        # Add solution step files
        for i, step in enumerate(quiz.get('solution', [])):
            step_num = step[0] if len(step) > 0 else f"{i+1}"
            quiz['html_files']['solution_steps'].append(
                f"{tag}_{question_number}_step_{step_num.replace('/', '_')}.html"
            )
        
        # Add solution image files
        for i, img_info in enumerate(quiz.get('solution_image_tag', [])):
            step_num = img_info[0] if len(img_info) > 0 else f"{i+1}"
            quiz['html_files']['solution_images'].append(
                f"{tag}_{question_number}_solution_{step_num.replace('/', '_')}.html"
            )
    ...
    return len(data['quizzes'])

def main():
    """Process Gr6_47_E1 and Gr6_47_E2 with proper separate files"""
    ...
    print("Creating PROPERLY SEPARATED HTML files for Gr6_47_E1 and Gr6_47_E2...")
    print("-" * 60)
    ...
    for json_file in exercises:
        ...
        # Update JSON with file references
        quiz_count = update_json_with_file_references(json_file)
        
        print(f"  Created {file_count} separate HTML files for {quiz_count} quizzes")
        print(f"  Updated {json_file} with file references")
        total_files += file_count
    
    print("\n" + "-" * 60)
    print(f"Total files created: {total_files}")
    print("Each file is COMPLETELY SEPARATE:")
    print("  - Main HTML: just question text")
    print("  - Question image HTML: standalone coordinate plane")
    print("  - Solution step HTMLs: individual step text")
    print("  - Solution image HTMLs: individual coordinate planes with annotations")
